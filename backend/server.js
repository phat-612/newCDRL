...
client.connect().then(() => {
  ...
  app.get("/new", checkIfUserLoginRoute, async (req, res) => {
    ...
    } catch (err) { console.log("SYSTEM | INDEX_NEW_ROUTE | ERROR | ", err); }
  });
  ...
  app.get("/", checkIfUserLoginRoute, async (req, res) => {
    ...
    } catch (err) { console.log("SYSTEM | INDEX_ROUTE | ERROR | ", err); }
  });
  ...
  app.get("/bancansu", checkIfUserLoginRoute, async (req, res) => {
    ...
    } catch (err) { console.log("SYSTEM | BAN_CAN_SU_INDEX_ROUTE | ERROR | ", err); }
  });
  ...
  app.post("/api/addOrEditTeachers", checkIfUserLoginAPI, async (req, res) => {
    try {
      ...
      if (user.pow[8]) {
        ...
        const curr_classes = await client.db(name_global_databases).collection('user_info').findOne(
          {
            _id: data.old_id,
          },
          {
