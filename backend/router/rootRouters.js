						...
						const studentActivities = await Promise.all(
							schoolYearsToSearch.map(async (year) => {
								...
								if (activitie_info_khoa) {
									if (
										activitie_info_khoa.ai &&
										activitie_info_khoa.ai[user._id]
									) {
								...
								if (activitie_info_truong) {
									if (
										activitie_info_truong.ai &&
										activitie_info_truong.ai[user._id]
									) {
		...
		try {
			...
			if (query.id && query.level) {
				...
				if (activitie_info.student_list) {
					...
					if (
						activitie_info.ai &&
						user._id in activitie_info.ai &&
						activitie_info.ai[user._id]
					) {
