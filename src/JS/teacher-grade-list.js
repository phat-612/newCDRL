...
$(document).on("click", ".auto_mark_btn", async function () {
  ...
  try {
    ...
    if (mssv_list.length > 0) {
      ...
      notify(
        "!",
        "Không có sinh viên được đánh dấu hoặc không đủ điều kiện để chấm điểm."
      );
    }
  } catch (error) {
    notify("x", "Có lỗi xảy ra!");
...
$(document).on("click", ".load_list_btn", async function () {
  ...
  try {
    ...
    const response = await fetch(
      `/api/teacher/loadScoresList?year=${year}&cls=${current_class}`,
      requestOptions
    );
    if (response.ok) {
                  ...
                  <td>
                    <div class="checkbox-wrapper-4">
                      <input type="checkbox" id="row${
                        i + 1
                      }" class="inp-cbx" value="${data.student_list[i]._id}" />
                      <label for="row${
                        i + 1
